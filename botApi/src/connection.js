...
async function getConnectionURL(ip) {
    const username = "cos4h";
    const password = "R@@t123";
    const port = "22";
    const usernameSSH = "vmonclick";
    const passwordSSH = "vmonclick";
    ...
    const selectorHost = ".connection-parameters input[type=text]";
    ...
    const selectorLabel = await page.evaluate((text) => {
        ...
        const label = labels.find((label) => label.textContent.includes(text));
    ...
    const connection = connections.find(
        (connection) => connection.innerText.trim() === nameConn
    );

    await browser.close();
    return connection ? connection.href : null;
}
