  ...
  public void saveMetricsIntance() {
    try {
      ...
      instances.stream().forEach(instance -> {
        ...
        try {
          data = vboxManage.getMetricsInstance(instance.getId());
          int cpu = data.get("CPU");
          int ram = data.get("RAM");
          int disk = data.get("DISK");
          int netTx = data.get("NET_TX");
          int netRx = data.get("NET_RX");
          MetricEntity metricEntity = MetricEntity.builder().idInstance(instance.getId()).cpu(cpu)
              .ram(ram).disk(disk).bandWidth(netTx).id(UUID.randomUUID())
