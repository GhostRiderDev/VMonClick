...
import { startInstance, stopInstance } from "@/services/instances";
import { AlertSuccess, AlertInfo } from "@/components/Alerts/Alerts";
import { getIp, getLink } from "@/services/conn";
...
const InitialVM = () => {
  ...
  const setConn = useStore((state: any) => state.setConn);
  ...
  const handleOnCLick = async (id: string, isStop: boolean) => {
    changeStatus(id);
    if (isStop) {
